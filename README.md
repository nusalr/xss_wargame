...
- 기본 페이지 : Search query에서 검색하는 경우 /vuln 페이지로 이동합니다.
- /memo : 클라이언트가 입력한 쿼리 파라미터 값을 화면에 출력합니다.
- /flag : Report 또는 Report to admin을 클릭하면 /flag 페이지로 이동합니다. 여기서 제출하는 값은 admin 봇이 /vuln 브라우저에 방문할 때 query parameter로 사용합니다.
- /submit : 획득한 플래그를 제출하는 페이지입니다.
- /success : 플래그가 올바른 값일 경우 이 페이지에 접근할 수 있습니다.

목표 : /vuln 페이지의 취약점을 확인하고, admin 봇이 javascript를 실행하도록 만들어 쿠키를 가져옵니다. 쿠키 값은 memo 페이지를 통해 확인하고, 얻은 flag 값을 submit 페이지에 제출해서 최종적으로 success 페이지에 도달하는 것이 목표입니다.
